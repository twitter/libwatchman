...
static size_t write_integer(json_t* json, stream_t* stream)
{
    ...
    if (i8 == v) {
        ...
        if (stream->write(stream, &tag, SIZE_U8) == SIZE_U8 &&
            stream->write(stream, &i8, SIZE_U8) == SIZE_U8) {
            bytes = SIZE_U8 + SIZE_U8;
        }
    } else {
        ...
        if (i16 == v) {
            tag = BSER_TAG_INT16;
            if (stream->write(stream, &tag, SIZE_U8) == SIZE_U8 &&
                stream->write(stream, &i16, SIZE_U16) == SIZE_U16) {
                bytes = SIZE_U8 + SIZE_U16;
            }
        } else {
            ...
            if (i32 == v) {
                tag = BSER_TAG_INT32;
                if (stream->write(stream, &tag, SIZE_U8) == SIZE_U8 &&
                    stream->write(stream, &i32, SIZE_U32) == SIZE_U32) {
                    bytes = SIZE_U8 + SIZE_U32;
                }
            } else {
                tag = BSER_TAG_INT64;
                if (stream->write(stream, &tag, SIZE_U8) == SIZE_U8 &&
                    stream->write(stream, &v, SIZE_U64) == SIZE_U64) {
                    bytes = SIZE_U8 + SIZE_U64;
                }
            }
        }
    }
...
static size_t write_compact_object(
    json_t* object, json_t* header, stream_t* stream)
{
    size_t bytes = 0;
...
static size_t write_compact_objects(
    json_t* objects, json_t* header, stream_t* stream)
{
    size_t bytes = 0;
    size_t obj_bytes = 1;
    size_t objs_bytes = 0;
...
static size_t null_stream_write(stream_t* s, const void* data, size_t nb)
{
    return nb;
}
...
static size_t buffer_stream_write(stream_t* s, const void* data, size_t nb)
{
    size_t result = 0;
...
static size_t file_stream_write(stream_t* s, const void* data, size_t nb)
{
...
static size_t write_header(size_t content_size, stream_t* stream)
{
...
static size_t write_pdu(json_t* root, size_t content_size, stream_t* stream)
{
    size_t hdr_bytes;
