...
static bser_t* new_unparsed(bser_buffer_t* buffer, bser_t* fill)
{
    if (fill == NULL) {
        fill = bser_alloc();
    }
...
static bser_t* new_error(const char* message, bser_t* fill)
{
    if (fill == NULL) {
        fill = bser_alloc();
    }
...
static bser_t* new_compact_object(
...
static bser_buffer_t* new_read_buffer(uint8_t* data, size_t buflen, int offset)
{
...
bser_t* bser_parse_content(uint8_t* data, size_t buflen, bser_t* fill)
{
...
{
    if (buflen < 2 || buffer[0] != '\0' || buffer[1] != '\1') {
...
static void parse_array(bser_t* fill, bser_buffer_t* buffer)
{
     bser_t length;
...
static void parse_object(bser_t* fill, bser_buffer_t* buffer)
{
     bser_t length;
     new_unparsed(buffer, &length);
     if (bser_is_integer(&length)) {
...
static void parse_string(bser_t* fill, bser_buffer_t* buffer)
{
     bser_t length;
     new_unparsed(buffer, &length);
     if (bser_is_integer(&length)) {
...
static void parse_compact_array(bser_t* fill, bser_buffer_t* buffer)
{
    bser_t header;
...
static int is_fully_parsed(bser_t* bser) {
    size_t index;
...
static void fully_parse(bser_t* bser) {
